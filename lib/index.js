...
*/
var compare = function compare(test, state) {
	if ((0, _helpers.isDictionary)(test) === true) {
		return compareDictionary(test, state);
	}

	if ((0, _helpers.isArray)(test) === true) {
...
function compareArray(test, state) {
	...
	if (!(0, _helpers.isArray)(state)) {
		return {
			passes: false,
			err: _util2.default.inspect(test, false, null) + ' is an array but ' + _util2.default.inspect(state, false, null) + ' is not'
		};
	}
...
function comparePrototypeArray(prototype, state) {
	...
	var r = compare(prototype, _underscore2.default.first(state));
	if (r.passes === false) {
		return r;
	}
...
function compareLeaf(test, state) {
	...
	if ((0, _helpers.isDictionary)(state) === false && (0, _helpers.isArray)(state) === false) {
		return { passes: true, err: undefined };
	} else {
		return {
			passes: false,
			err: _util2.default.inspect(test, false, null) + ' is a leaf but ' + _util2.default.inspect(state, false, null) + ' is not'
		};
	}
}
...
function compareDictionary(test, state) {
	...
	if ((0, _helpers.containsKey)(testKey, state) === undefined) {
		return {
			passes: false,
	...
	var sameValue = compare((0, _helpers.get)(testKey, test), (0, _helpers.get)(testKey, state));
	if (sameValue.passes === false) {
		return sameValue;
	}

	// values are the same. Remove testKey and recurse with rest of map
	var trimmedTest = (0, _helpers.copyWithoutKey)(testKey, test);
	var trimmedState = (0, _helpers.copyWithoutKey)(testKey, state);
